...
void RenderHardwareInterface::ClearSceneBuffer(float val, int /*flags*/, bool depthOnly) const
{
	if (!depthOnly)
		myContext->ClearRenderTargetView(mySceneBuffer->myRTV.Get(), mySceneBuffer->myClearColor.data());

	myContext->ClearDepthStencilView(mySceneBuffer->myDSV.Get(), D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, val, 0);
}
...
void RenderHardwareInterface::SetRenderTargetView(Microsoft::WRL::ComPtr<ID3D11RenderTargetView> aRTV, ID3D11DepthStencilView* aDSV)
{
	myContext->OMSetRenderTargets(1, aRTV.GetAddressOf(), aDSV);
...
void RenderHardwareInterface::SetDefferedBufferRT(ID3D11RenderTargetView* aRTV)
{
	...
	myContext->OMSetRenderTargets(6, rtvs.data(), mySceneBuffer->myDSV.Get());
