...
//std::unordered_map<const char*, unsigned> myShaders;

#ifdef OPENGL

bool ShaderCompiler::CompileShader(const char* aPath, unsigned& outShader, ShaderType aType)
{
    //if (myShaders.contains(aPath))
    //    return myShaders[aPath];
    ...
    if (!success)
    {
        ...
        {
        case ShaderCompiler::FragmentShader:
            std::cout << "ERROR::SHADER::FRAGMENT/PIXEL::COMPILATION_FAILED - shader: " << aPath << std::endl << infoLog << std::endl;
            break;
        case ShaderCompiler::VertexShader:
            std::cout << "ERROR::SHADER::VERTEX::COMPILATION_FAILED - shader: " << aPath << std::endl << infoLog << std::endl;
            break;
        case ShaderCompiler::GeometryShader:
            std::cout << "ERROR::SHADER::GEOMETRY::COMPILATION_FAILED - shader: " << aPath << std::endl << infoLog << std::endl;
            break;
        }
