...
std::unordered_map<const char*, Shader> globalShadersLoaded;

Shader::Shader(const char* aPath, ShaderCompiler::ShaderType aType) : myPath(aPath)
{
	if (globalShadersLoaded.find(aPath) != globalShadersLoaded.end())
		*this = globalShadersLoaded[aPath];
	else
	{
		ShaderCompiler::CompileShader(aPath, myID, aType);
		globalShadersLoaded[aPath] = *this;
	}
...
{
	glAttachShader(aProgramID, myID);
}
