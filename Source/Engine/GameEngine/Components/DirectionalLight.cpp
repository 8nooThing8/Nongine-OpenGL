...
DirectionalLight::DirectionalLight() : myIntensity(1.f), myColor(1, 1, 1), myCamera(false, { 1024 , 1024 })
{

}

void DirectionalLight::BindLightFrameBuffer()
{
	myCamera.Use();

	GLsizei shadowSizeUInt = static_cast<GLsizei>(myShadowTextureSize);

	glViewport(0, 0, shadowSizeUInt, shadowSizeUInt);
	glBindFramebuffer(GL_FRAMEBUFFER, myShadowMapFrameBuffer);
	glClear(GL_DEPTH_BUFFER_BIT);
}

void DirectionalLight::Start()
{
	myCamera.position = gameObject->transform->GetPosition();
	myCamera.rotation = CU::Vector4f(-45, 45, 0, 0.f);

	myCamera.Update(0.f);

	myCamera.position -= CU::Vector4f(myCamera.GetForward() * 250.f, 1.f);

	Material::SetGlobalInt("shadowMap", 5);

	Material::SetGlobalFloat("light.intensity", myIntensity);
	Material::SetGlobalVector3("light.lightColor", myColor);
	Material::SetGlobalVector3("light.lightDir", myCamera.GetForward());

	glGenFramebuffers(1, &myShadowMapFrameBuffer);

	glGenTextures(1, &myShadowMapTexture);
	glBindTexture(GL_TEXTURE_2D, myShadowMapTexture);

	GLsizei shadowSizeUInt = static_cast<GLsizei>(myShadowTextureSize);

	glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, shadowSizeUInt, shadowSizeUInt, 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL);
	...
	glBindFramebuffer(GL_FRAMEBUFFER, myShadowMapFrameBuffer);
		...
		glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, myShadowMapTexture, 0);
#else
		glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, myShadowMapTexture, 0);
#endif
...
void DirectionalLight::EarlyUpdate(float)
{
	if (myCamera.GetIsDirty())
	{
		myCamera.Update(0.f);
		myCamera.RemoveIsDirty();
	}
}

void DirectionalLight::SetColor(const CU::Vector3f& aColor)
{
	myColor = aColor;
	Material::SetGlobalVector3("light.lightColor", myColor);
}

void DirectionalLight::SetIntensity(float aIntensity)
{
	myIntensity = aIntensity;
	Material::SetGlobalFloat("light.lightColor", myIntensity);
}
...
void DirectionalLight::HandleSave(rapidjson::Value& outJson, rapidjson::Document& document)
{
	rapidjson::Value dirLightIntensity(rapidjson::kNumberType);
	float lightIntensity = myIntensity;

	dirLightIntensity.SetFloat(lightIntensity);

	rapidjson::Value dirLightColor(rapidjson::kArrayType);
	dirLightColor.PushBack(myLightColor.x, document.GetAllocator());
	dirLightColor.PushBack(myLightColor.y, document.GetAllocator());
	dirLightColor.PushBack(myLightColor.z, document.GetAllocator());
	dirLightColor.PushBack(myLightColor.w, document.GetAllocator());

	outJson.AddMember("Intensity", dirLightIntensity, document.GetAllocator());
	outJson.AddMember("Color", dirLightColor, document.GetAllocator());
}

void DirectionalLight::HandleLoad(const rapidjson::Value& aJson)
{
	Vector4<float> color{
		aJson["Color"][0].GetFloat(),
		aJson["Color"][1].GetFloat(),
		aJson["Color"][2].GetFloat(),
		aJson["Color"][3].GetFloat()
	};

	myLightColor = color;
	myIntensity = aJson["Intensity"].GetFloat();
}

DirectionalLight::DirectionalLight() : myIntensity(1), myLightColor(CommonUtilities::Vector4<float>(1, 1, 1, 1))
{
	Component();
...
inline void DirectionalLight::EarlyUpdateEditor(float)
{
	...
	DefferedRendering::Get().RenderAllMeshes();
