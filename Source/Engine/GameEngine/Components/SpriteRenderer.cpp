...
void SpriteRenderer::HandleSave(rapidjson::Value& aOutJson, rapidjson::Document& aDocument)
{
    auto sprite = rapidjson::Value(mySprite->mytexturePath.string().c_str(), aDocument.GetAllocator());
    aOutJson.AddMember("Sprite", sprite, aDocument.GetAllocator());
}

void SpriteRenderer::HandleLoad(const rapidjson::Value& aObj)
{
    mySprite = new Sprite(aObj["Sprite"].GetString());
}
