...
Ray<float> GameCamera::CameraPointToRay(const Vector2f& aScreenCoords)
{
	...
	ndc.x = CU::Lerp(-1.f, 1.f, ndc.x / (myScreenResolution.x - 1.f));
	ndc.y = CU::Lerp(-1.f, 1.f, ndc.y / (myScreenResolution.y - 1.f));
...
void CommonUtilities::GameCamera::Update(float)
{
...
void CommonUtilities::GameCamera::Use()
{
	Material::SetGlobalMatrix4x4("projectionMatrix", GetProjectionMatrix());
	Material::SetGlobalMatrix4x4("invViewMatrix", GetInverse());
	Material::SetGlobalVector3("viewPos", position);
}

const CommonUtilities::Matrix4x4<float>& GameCamera::GetInverse() const
{
	return myInverse;
}
