...
ID3D11ShaderResourceView* PostProcessing::Render(ID3D11ShaderResourceView* inSRV)
{
	...
	MainSingleton::Get()->shaderResources->AddShaderResource(new ShaderResourceItem(81, 1, textures[0]->rtvSRV.GetAddressOf()));
	...
	GraphicsEngine::GetRHI()->GetContext()->PSSetShaderResources(81, 1, textures[0]->rtvSRV.GetAddressOf());
	...
	GraphicsEngine::GetRHI()->GetContext()->PSSetShaderResources(81, 1, textures[1]->rtvSRV.GetAddressOf());
	...
	GraphicsEngine::GetRHI()->GetContext()->PSSetShaderResources(81, 1, textures[2]->rtvSRV.GetAddressOf());
	...
	GraphicsEngine::GetRHI()->GetContext()->PSSetShaderResources(81, 1, textures[3]->rtvSRV.GetAddressOf());
	textures[4]->spriteRenderer->LateUpdateEditor(0);

	GraphicsEngine::GetRHI()->SetRenderTargetView(textures[5]->rtv);
	GraphicsEngine::GetRHI()->GetContext()->PSSetShaderResources(81, 1, textures[4]->rtvSRV.GetAddressOf());
	textures[5]->spriteRenderer->LateUpdateEditor(0);

	MainSingleton::Get()->shaderResources->AddShaderResource(new ShaderResourceItem(82, 1, textures[5]->rtvSRV.GetAddressOf()));
	...
	return textures[textures.size() - 1]->rtvSRV.Get();
	//return inSRV;
}
...
void PostProcessing::Init(const HWND aHWND)
{
	...
	for (int i = 0; i < textures.size(); i++)
	{
		...
		hr = device->CreateShaderResourceView(texture, nullptr, textures[i]->rtvSRV.GetAddressOf());

		name = textures[i]->typeToString();
		textures[i]->rtvSRV->SetPrivateData(WKPDID_D3DDebugObjectName, static_cast<UINT>(name.length()), name.c_str());
