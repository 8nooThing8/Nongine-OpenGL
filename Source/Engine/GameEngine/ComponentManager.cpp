...
void ComponentManager::CleanUp()
{
	...
	{
		auto meshRend = dynamic_cast<MeshRenderer*>(meshRenderer);

		meshRend->ShaderCleanUp();
	}
}
...
void ComponentManager::RenderMeshOverride(MeshRenderer* notRenderThis, Microsoft::WRL::ComPtr<ID3D11PixelShader> shaderOverride)
{
	for (auto& meshRenderer : myComponents["MeshRenderer"])
	{
		...
		{
			auto meshRendererComp = static_cast<MeshRenderer*>(meshRenderer);

			if (meshRendererComp->myCastShadow)
			{
				meshRendererComp->Draw(shaderOverride);
			}
		}
	}
}

std::vector<GameObject*> ComponentManager::GetGameObjectsOnLayer(int aLayer)
{
	std::vector<GameObject*> gameobjects;

	for (auto& meshRenderer : myComponents["MeshRenderer"])
	{
		auto mRenderer = static_cast<MeshRenderer*>(meshRenderer);

		if (mRenderer->myLayer == aLayer)
			gameobjects.emplace_back(mRenderer->gameObject);
	}

	return gameobjects;
}

void ComponentManager::RenderRemoveMeshesLayer(int aLayer)
{
	for (auto& meshRenderer : myComponents["MeshRenderer"])
	{
		auto mRenderer = static_cast<MeshRenderer*>(meshRenderer);

		if (mRenderer->myLayerToRemove == aLayer && mRenderer->myLayerToRemove > 0)
			mRenderer->Draw();
	}
}

void ComponentManager::RenderMeshesOnLayer(int aLayer)
{
	for (auto& meshRenderer : myComponents["MeshRenderer"])
	{
		auto mRenderer = static_cast<MeshRenderer*>(meshRenderer);

		if (mRenderer->myLayer == aLayer && mRenderer->myLayerToRemove < 1)
			mRenderer->Draw();
	}
}
