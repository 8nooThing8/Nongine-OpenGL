...
namespace CommonUtilities
{
	...
	public:
		...
		{
			std::memcpy(myData, aMatrix.myData, 64);
		}
		...
		inline Vector4<T> GetRotation() const
		{
			...
			if (Abs(sinX) > 0.9999f)
			{
				rotation.y = std::atan2f(-matrix(1, 3), matrix(1, 1));
				rotation.z = 0.0f;
			}
			else
			{
				rotation.y = std::atan2f(matrix(3, 1), matrix(3, 3));
				rotation.z = std::atan2f(matrix(1, 2), matrix(2, 2));
			}
		...
		static inline CU::Matrix4x4<T> LookAt(CU::Vector3f aForward, CU::Vector3f aCenter, CU::Vector3f aUp)
		{
			CU::Matrix4x4<T> mat;
