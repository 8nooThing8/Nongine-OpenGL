...
namespace CommonUtilities
{
	...
	public:
		...
		Vector3(const OtherVectorClass& aVector) : x(static_cast<T>(aVector.x)), y(static_cast<T>(aVector.y)), z(static_cast<T>(aVector.z))
		{
		}
	...
	template <class T>
	T DistanceSqrd(const Vector3<T>& aVector0, const Vector3<T>& aVector1)
	{
		const T xDistance = aVector0.x - aVector1.x;
		const T yDistance = aVector0.y - aVector1.y;
		const T zDistance = aVector0.z - aVector1.z;
