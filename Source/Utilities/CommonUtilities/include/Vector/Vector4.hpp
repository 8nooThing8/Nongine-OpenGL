...
namespace CommonUtilities
{
	...
	public:
		...
		Vector4<T>(const std::initializer_list<T>& initList)
		{
			x = *(initList.begin());
			y = *(initList.begin() + 1);
			z = *(initList.begin() + 2);
		...
		//Creates a vector (aX, aY, aZ, aW)
		Vector4<T>(const Vector3<T>& aVector, T aW = static_cast<T>(1))
		{
			x = aVector.x;
			y = aVector.y;
			z = aVector.z;
	...
	{
		if (aVector0.x != aVector1.x) return false;
		if (aVector0.y != aVector1.y) return false;
		if (aVector0.z != aVector1.z) return false;
		if (aVector0.w != aVector1.w) return false;
