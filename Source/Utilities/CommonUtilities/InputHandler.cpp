...
{
	bool returnValue = !myPrevState[aKeyCode] && myCurrState[aKeyCode];

	if (returnValue)
	{
		myPrevState[aKeyCode] = myCurrState[aKeyCode];
	}
...
bool InputHandler::GetMouseButtonDown(const int aMouseButtonIndex)
{
	...
	{
	case 0:
		returnvalue = myPrevState[keycode::MOUSELBUTTON] == false && myCurrState[keycode::MOUSELBUTTON] == true;

		myPrevState[keycode::MOUSELBUTTON] = myCurrState[keycode::MOUSELBUTTON];

		return returnvalue;
	case 1:
		returnvalue = myPrevState[keycode::MOUSERBUTTON] == false && myCurrState[keycode::MOUSERBUTTON] == true;

		myPrevState[keycode::MOUSERBUTTON] = myCurrState[keycode::MOUSERBUTTON];

		return returnvalue;
	case 2:
		returnvalue = myPrevState[keycode::MBUTTON] == false && myCurrState[keycode::MBUTTON] == true;

		myPrevState[keycode::MBUTTON] = myCurrState[keycode::MBUTTON];
...
bool InputHandler::GetMouseButton(const int aMouseButtonIndex)
{
	...
	{
	case 0:
		return myCurrState[keycode::MOUSELBUTTON];
	case 1:
		return myCurrState[keycode::MOUSERBUTTON];
	case 2:
		return myCurrState[keycode::MBUTTON];
	}
...
bool InputHandler::MousePressed(int aIndex) const
{
	return true;
}
...
void InputHandler::UpdateInput()
{
	...
	unsigned monitor_width = info.rcMonitor.right - info.rcMonitor.left;
	unsigned monitor_height = info.rcMonitor.bottom - info.rcMonitor.top;
