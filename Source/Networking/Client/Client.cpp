...
int Client::Init()
{
	...
	inet_pton(AF_INET, "84.217.51.22", &clientAddr.sin_addr.s_addr);
	
	//std::cout << "ip: ";
	std::string ipString = "84.217.51.22";
...
{
	assert(aDataSent != DataTypeSent::NONE && ", data type to send is set to none on the component im trying to send");
	...
	memcpy(valueToCopyInto + 8, charData, aBytesToSend);

	memcpy(valueToCopyInto, &type, sizeof(int));
	memcpy(valueToCopyInto + 4, &aGOID, sizeof(int));

	int iResult = sendto(mySocket, valueToCopyInto, aBytesToSend + 8, 0, (SOCKADDR*)&serverAddr, (int)sizeof(serverAddr));
...
NetworkRecieveData Client::RecieveData()
{
	...
	if (iResult > 0)
	{
		...
		NetworkRecieveData data{ dataTypeInt, iResult, objIDInt };

		std::memcpy(data.data, recbuf + sizeof(int) * 2, iResult);
