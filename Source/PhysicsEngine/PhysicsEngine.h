...
constexpr int collisionLayerCount = 5;
...
enum class CollisionLayer : physx::PxU32
{
	...
	Trigger = 1 << 3,
	Cabin = 1 << 4,
	IgnoreRaycast = 1 << collisionLayerCount,
	All = (1 << collisionLayerCount) - 1
};

class ContactModify : public PxContactModifyCallback
{
	void onContactModify(PxContactModifyPair* const pairs, PxU32 aCount) override;
};
...
struct Hit
{
	...
	CommonUtilities::Vector3<float> localNormal;	// Local-space hit normal
...
public:
	 ...
	 *
	 */
	physx::PxShape* CreateMeshCollider(TGA::FBX::Mesh* aMesh, float scale) const;
	/**
	 *Create Box collider
	 *
	...
	*/
	...
	static bool RayCast(const CommonUtilities::Ray<float>& aRay, Hit& outHits, float maxDistance = 10000000.f, physx::PxU32 aLayers = static_cast<physx::PxU32>(CollisionLayer::All), bool aDebugDrawRay = false);
	static bool RayCasts(const CommonUtilities::Ray<float>& aRay, Hits& outHits, float maxDistance = 10000000.f, physx::PxU32 aLayers = static_cast<physx::PxU32>(CollisionLayer::All), bool aDebugDrawRay = false);
	...
	void UpdateRigidBody(RigidBody* aRigidBody);
...
private:
	...
	std::vector<RigidBody*> myRigidBodiesToBeUpdated;
};