...
/*** TODO: Functions to implement for BRDF ***/
...
//float3 Fresnel_SphericalGaussianSchlick(float3 aToView, float3 aHalfangle, float3 aF0)
//float3 fresnelSchlickRoughness(float cosTheta, float3 F0, float roughness)
//float3 Specular_BRDF(float aRoughness, float3 aNormal, float3 aHalfAngle, float3 aToView, float3 aToLight, float3 aF0)
...
float4 IntegrateBRDF(float aNdotV, float aRoughness)
{
    float3 V;
    ...
    return float4(float2(a, b) / NUM_SAMPLES, 0, 1);
}