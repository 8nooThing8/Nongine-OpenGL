...
void ParticleSystem::LateUpdateEditor(float)
{
	...
	RenderParticleCommand command = RenderParticleCommand(myParticleRenderer.get());

	MainSingleton::Get()->myForwardCommandList->Enqueue<RenderParticleCommand>(command);
}
