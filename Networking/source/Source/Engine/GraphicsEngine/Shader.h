...
public:
	...
	void SetDiffuseName(std::string& aDiffuseName) { myDiffuseName = aDiffuseName; }
	void SetNormalName(std::string& aNormalName) { myNormalName = aNormalName; }
	void SetRoughnessName(std::string& aRoughnessName) { myRoughnessName = aRoughnessName; }
	void SetMetalliclName(std::string& aMetallicName) { myMetallicName = aMetallicName; }
	void SetAOName(std::string& aAOName) { myAOName = aAOName; }

	void SetDiffusePath  (std::string& aDiffusePath)   { myDiffuseTexturePath = aDiffusePath;   }
	void SetNormalPath   (std::string& aNormalPath)    { myNormalTexturePath = aNormalPath;    }
	void SetRoughnessPath(std::string& aRoughnessPath) { myRoughnessTexturePath = aRoughnessPath; }
	void SetMetalliclPath(std::string& aMetallicPath)  { myMetallicTexturePath = aMetallicPath;  }
	void SetAOPath       (std::string& aAOPath)        { myAOTexturePath = aAOPath;        }
private:
