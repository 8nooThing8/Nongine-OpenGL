...
namespace
{
    struct handle_closer { void operator()(const HANDLE h) noexcept { if (h) CloseHandle(h); } };

    using ScopedHandle = std::unique_ptr<void, handle_closer>;

    inline HANDLE safe_handle(const HANDLE h) noexcept { return (h == INVALID_HANDLE_VALUE) ? nullptr : h; }
    ...
    //--------------------------------------------------------------------------------------
    size_t BitsPerPixel(_In_ const DXGI_FORMAT fmt) noexcept
    {
        switch (fmt)
        {
...
#undef ISBITMASK
    ...
    //--------------------------------------------------------------------------------------
    DXGI_FORMAT MakeSRGB(_In_ const DXGI_FORMAT format) noexcept
    {
        switch (format)
        {
        case DXGI_FORMAT_R8G8B8A8_UNORM:
    ...
    //--------------------------------------------------------------------------------------
    inline DXGI_FORMAT MakeLinear(_In_ const DXGI_FORMAT format) noexcept
    {
        switch (format)
        {
        case DXGI_FORMAT_R8G8B8A8_UNORM_SRGB:
    ...
    HRESULT FillInitData(
        ...
        _In_ const size_t mipCount,
        _In_ const size_t arraySize,
        _In_ const DXGI_FORMAT format,
        _In_ const size_t maxsize,
        _In_ const size_t bitSize,
...
HRESULT DirectX::CreateDDSTextureFromMemoryEx(
    ...
    const DDS_LOADER_FLAGS loadFlags,
    ID3D11Resource** texture,
    ID3D11ShaderResourceView** textureView,
    DDS_ALPHA_MODE* alphaMode) noexcept
{
